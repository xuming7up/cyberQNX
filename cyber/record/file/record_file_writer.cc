...
* @Version: 1.0.0
* @Feature List:
* -1. The cyber_record tool will be crashed when recording data, 
*     because of the low I/O efficiency on some hardware platform.
*     We apply a workaround method to mitigate this problem by dropping some data frames when the I/O blocked.
* -2. The async Flush task will cause the final close() flush function 
*     write a new empty chunk which has an error End timestamp (1970-01-01) 
*     into the record file.
*     We add an empty check to elimit this problem.
*
*/
...
bool RecordFileWriter::WriteMessage(const proto::SingleMessage& message) {
  ...
  if (flush_task_.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready) {
    flush_task_ = std::async(
        std::launch::async,
        [this, chunk = std::move(chunk_active_)]() { this->Flush(*chunk); });
  } else {
    AERROR << "Flushing didn't finish. Either the hardware cannot keep up or the "
           << "flush rate is too fast."
           << "just skip this frame for a workaround!!!";
  }
...
void RecordFileWriter::Flush(const Chunk& chunk) {
  if(chunk.header_.end_time() == 0 || chunk.header_.message_number()==0){
    //AINFO << "Ignored empty chunk: " << chunk.header_.DebugString();
    return;
  }
