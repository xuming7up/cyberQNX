...
#ifdef __QNX__
QFileType GetQFileType(const char* dir,const char* entry_name){
    char full_path[PATH_MAX];
    std::memset(full_path,'\0',PATH_MAX);
    snprintf(full_path, sizeof(full_path), "%s/%s", dir, entry_name);

    QFileType qnx_type = QFileType::DT_UNKNOWN;

    struct stat stat_buf;
    if (lstat(full_path, &stat_buf) != 0){
      AWARN << "Failed to lstat: " << full_path;
      return qnx_type;
    }

    if (S_ISLNK(stat_buf.st_mode)){
      qnx_type = QFileType::DT_LNK;
    }
    else if (S_ISDIR(stat_buf.st_mode)){
      qnx_type = QFileType::DT_DIR;
    }
    else if (S_ISREG(stat_buf.st_mode)){
      qnx_type = QFileType::DT_REG;
    }
    else{
      AWARN << "Failed to get type: " << full_path;
    }
...
bool CopyDir(const std::string &from, const std::string &to) {
  ...
  if (EnsureDirectory(to)) {
    ...
    while ((entry = readdir(directory)) != nullptr) {
      ...
      uint8_t type;
#ifdef __QNX__
      type = GetQFileType(from.c_str(),entry->d_name);
#else
      type = entry->d_type;
#endif
      if (type == DT_DIR) {
        ...
        ret &= CopyFile(sub_path_from, sub_path_to);//Issue: 目录里的link会被拷贝成一个文件；如果后续在调用的地方发现问题应做bug处理修复
      }
    }
  } else {
...
std::vector<std::string> ListSubPaths(const std::string &directory_path,
  ...
  while ((entry = readdir(directory)) != nullptr) {
    ...
    uint8_t type;
  #ifdef __QNX__
    type = GetQFileType(directory_path.c_str(),entry->d_name);
  #else
    type = entry->d_type;
  #endif
  if (type == d_type && strcmp(entry->d_name, ".") != 0 &&
