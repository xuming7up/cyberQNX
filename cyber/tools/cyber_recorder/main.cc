...
* @Version: 1.0.0
* @Feature List:
* -1.Sanity check to make sure channels ending with "../image" have a corresponding "../h264_compressed" channel for h264 decoding 
*/
...
int main(int argc, char** argv) {
  ...
  if (command == "play") {
    ...
    for (const auto &ch : play_param.channels_to_play) {
      if ((ch.substr(ch.find_last_of('/') + 1) == "image")) {
        if (play_param.channels_to_play.count(ch + std::string{"/h264_compressed"}) == 0) {
          play_param.channels_to_play.insert(ch + std::string{"/h264_compressed"});
        }
      }
    }
