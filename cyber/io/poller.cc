...
void Poller::Poll(int timeout_ms) {
  ...
  for (auto &req : requests_) {
    auto req_ptr = req.second;
    m_pollfds.push_back(req_ptr->pfd);
  }
  if (poll(&m_pollfds[0], m_pollfds.size(), timeout_ms) < 0) {
    if (errno == EINTR) {
      ADEBUG << "poll() interrupted by a signal";
    }
    AERROR << "poll() failed, " << strerror(errno);
    return;
  }
  int ready_num{0};
  std::unordered_map<int, PollResponse> responses;
  for (size_t i{0}; i < m_pollfds.size(); ++i){
    responses[m_pollfds[i].fd] = PollResponse(m_pollfds[i].revents);
  }
