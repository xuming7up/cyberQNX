...
* @Version: 1.0.0
* @Feature List:
* -1.Add some specific implementation details of function Block(int timeout_ms, bool is_read) and Fill(int timeout_ms, bool is_read) on qnx system. 
*/
...
void PollHandler::Fill(int timeout_ms, bool is_read) {
  ...
  if (is_read) {
    struct pollfd pfd
    {
      request_.fd, POLLIN, 0
    };
    request_.pfd = pfd;
  } else {
    struct pollfd pfd
    {
      request_.fd, POLLOUT, 0
    };
    request_.pfd = pfd;
  }
#else
