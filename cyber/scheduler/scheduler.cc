...
* @Feature List:
* -1. add new function:
*     void SetInnerThreadAttr(const std::string& name, pthread_t p_tid);
* 
*/
...
void Scheduler::SetInnerThreadAttr(const std::string& name, pthread_t p_tid){
  //AINFO << "SetInnerThreadAttr: " << name << ",threadId:" << p_tid << "..."; 
  if (inner_thr_confs_.find(name) != inner_thr_confs_.end()) {
    auto th_conf = inner_thr_confs_[name];
    auto cpuset = th_conf.cpuset();
    //AINFO << "SetInnerThreadAttr threadId:" << p_tid << ", cpuset: " << cpuset;

    std::vector<int> cpus;
    ParseCpuset(cpuset, &cpus);
    SetSchedAffinity(p_tid, cpus, "range");
    SetSchedPolicy(p_tid, th_conf.policy(), th_conf.prio());
  }
}
