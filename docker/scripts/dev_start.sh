...
product=""
necessary_tf_cuda_compute_capabilities=""
if [ "${TARGET_ARCH}" = "aarch64" ]; then
	product="$(sudo lshw -c system -json | grep -Po '(?<="product" : ")[^"]+' | head -n 1)"
    # pegasus and proAI
    # this variable will be appended to the native CUDA compute capability, only on aarch64
    # eg, if a machine with 8.6 native CUDA compute capability, then it's final CUDA compute capability will be '7.2,7.5,8.6'
    # which means the CUDA compiled on this machine can be run on machine with 7.2, 7.5 and 8.6 GPU architecture.
    necessary_tf_cuda_compute_capabilities="7.2,7.5"
else
	product="NOCARE"
    # RTX4000 and GTX3080ti
fi
...
function stop_all_apollo_containers_for_user() {
    ...
    if [ -d "/sys/fs/cgroup/cpuset/system" ]; then
            source ${APOLLO_ROOT_DIR}/scripts/tune_performance.sh 
    fi
}
...
function determine_dev_image() {
    ...
    echo "GITUSERNAME=${gitusername}" > ./.settings
    echo "GITUSEREMAIL=${gituseremail}" >> ./.settings
}
...
function setup_devices_and_mount_local_volumes() {
    ...
    # ProAI_Gen20 : Proai  | e3550_t194 : pegsus | p3710-0010 : drive orin | Orin : jetson orin ----
    if [ "${TARGET_ARCH}" = "aarch64" ] && [[ $product =~ .*"ProAI_Gen20" ]] || [[ $product =~ .*"e3550_t194"* ]]; then
	    volumes="${volumes} -v /usr/lib:/usr/lib/aarch64-linux-gnu/tegra:ro -v /etc/tacp:/etc/tacp:ro -v /usr/local/driveworks-2.2/:/usr/local/driveworks-2.2/:ro"
    fi
    if [ "${TARGET_ARCH}" = "aarch64" ] && [ "$product" = "p3710-0010" ]; then
            volumes="${volumes} -v /usr/lib:/usr/lib/aarch64-linux-gnu/tegra:ro -v /usr/lib/nvsipl_drv:/usr/lib/nvsipl_drv:ro -v /opt/nvidia/nvmedia/nit:/opt/nvidia/nvmedia/nit:ro
                    -v /usr/local/driveworks-5.4/:/usr/local/driveworks-5.4/:ro"
    fi
    if [ "${TARGET_ARCH}" = "aarch64" ] && [[ "$product" =~ .*"Orin"* ]]; then
	    volumes="${volumes} -v /usr/lib/aarch64-linux-gnu/tegra:/usr/lib/aarch64-linux-gnu/tegra:ro"
    fi
...
function mount_other_volumes() {
    ...
    if [ ${TARGET_ARCH} == "x86_64" ]; then
        ...
        echo "not FOUND data_volume-audio_model-latest IMAGE!"
	exit 0
    fi
    ...
    if [ ${TARGET_ARCH} == "x86_64" ]; then
        ...
        echo "not FOUND yolov4_volume-emergency_detection_model-latest IMAGE!"
        exit 0
    fi
    ...
    if [ ${TARGET_ARCH} == "x86_64" ]; then
        ...
        echo "not FOUND faster_rcnn_volume-traffic_light_detection_model-latest IMAGE!"
        exit 0
    fi
...
function post_run_setup() {
    if [ "${USER}" != "root" ]; then
    if [ "${TARGET_ARCH}" = "aarch64" ] && [[ "$product" =~ .*"ProAI_Gen20" ]] || [[ "$product" =~ .*"e3550_t194"* ]] || [ "$product" = "p3710-0010" ]; then
	    DOCKER_USER="${user}"
	    DOCKER_USER_ID=$(id -u)  #"${uid}"
	    DOCKER_GRP="$(id -g -n)" #${grout}
	    DOCKER_GRP_ID="$(id -g)" #"${gid}"
            if [ ${tune_performance} == "yes" ]; then
                if [ ! -d "/sys/fs/cgroup/cpuset/system" ]; then
                    source ${APOLLO_ROOT_DIR}/scripts/tune_performance.sh 1
                fi
            else 
                if [ -d "/sys/fs/cgroup/cpuset/system" ]; then
                    source ${APOLLO_ROOT_DIR}/scripts/tune_performance.sh
            ...
            docker-exec ${APOLLO_DEV} bash /apollo/scripts/docker_start_user.sh ${DOCKER_USER} ${DOCKER_USER_ID} ${DOCKER_GRP} ${DOCKER_GRP_ID} ${product} ${tune_performance} ${network_interface} ${necessary_tf_cuda_compute_capabilities}
...
function main() {
	...
	-e product="${product}" \
    -e tune_performance=${tune_performance} \
    -e network_interface=${network_interface} \
    -e necessary_tf_cuda_compute_capabilities=${necessary_tf_cuda_compute_capabilities} \
        ${MAP_VOLUMES_CONF}      \
        ${OTHER_VOLUMES_CONF}    \
        ${local_volumes}        \
        --net host \
