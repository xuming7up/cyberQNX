...
void MainWindow::PointCloudReaderCallback(
  ...
  pointcloud_reader_mutex_.unlock();
  uint32_t size = 0;
  std::vector<const apollo::drivers::PointCloud*> pnt_cloud_vec;
  if(pdata->merged_point_clouds().size() > 0){
    for(auto& p : pdata->merged_point_clouds()){
      size += p.width();
      pnt_cloud_vec.emplace_back(&p);
    }
    //std::cout << "-----Merged point clouds: " << pnt_cloud_vec.size() << " points: " << size << std::endl;
  }else{
    size = pdata->width();
    pnt_cloud_vec.emplace_back(pdata.get());
  }
  PointCloud* pc = new PointCloud(size, 4, pointcloud_shader_);
  if (pc) {
    if (!pc->FillData(pnt_cloud_vec) ||
